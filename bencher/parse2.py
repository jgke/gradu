...
data = {}
languages = ['purkka', 'gpp', 'rust', 'gnat', 'go']

output = ["name,lang,id,n,size(B),cpu(s),mem(KB),status,load,elapsed(s),busy(s)"]
...
for benchmark in data:
    ...
    output += [",".join(list(map(lambda x: str(x), best.values())))]
...
for benchmark in data:
    ...
    for language in languages:
        ...
        for language in languages:
            ...
            row = min(this[biggest_n], key=(lambda x: float(x["elapsed(s)"])))
            for cur_row in this[biggest_n]:
                output += [",".join(list(map(lambda x: str(x), cur_row.values())))]
